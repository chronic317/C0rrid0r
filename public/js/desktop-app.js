...
(function () {
  ...
  function renderNearby(a) {
    ...
    grid.innerHTML = a.cams.map(cid => {
      ...
      const imgSrc  = liveCam
        ? '/api/camproxy?url=' + encodeURIComponent(liveCam.imageUrl)
        : (fallCam ? CAM(fallCam.img) : '');
  ...
  function loadStillFromLive(cam, img, ph, liveTag) {
    ...
    // Use ?url= path when we have an upstream imageUrl from cameralocations,
    // otherwise fall back to legacy ?id= path for FALLBACK_CAMERAS.
