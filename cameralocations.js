...
// ── ROUTE NORMALIZATION ──────────────────────────────────────────────────────
// Different feeds spell routes differently (I-80, I-90, I-80/90, I80, etc).
// Normalize to the form the frontend uses for matching.
function normalizeRoute(r) {
  if (!r) return 'UNKNOWN';
  const s = r.toUpperCase().replace(/\s+/g, '');
  if (/^I-?80\/?90$|^I-?90\/?80$/.test(s)) return 'I-80/90';
  if (/^I-?80$/.test(s)) return 'I-80/90';   // toll road in IN is shared
  if (/^I-?90$/.test(s)) return 'I-80/90';
  const m = s.match(/^(I|US|SR|IN)-?(\d+)$/);
  if (m) return `${m[1]}-${m[2]}`;
  return r.trim();
}
...
function fetchCARSFeed() {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(10000, () => { req.destroy(); reject(new Error('CARS feed timed out')); });
    req.on('error', reject);
    req.end();
  });
}
      ...
      res.on('end', () => {
        try {
          ...
          const cameras = views.map(cam => ({
            id:        cam.uri,
            title:     cam.title,
            route:     normalizeRoute(cam.parentCollection?.location?.routeDesignator),
            imageUrl:  cam.url,
